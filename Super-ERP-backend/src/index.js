...
app.options('/*splat', corsMiddleware); app.use(express.json()); app.use(express.urlencoded({extended:true})); app.use(require('./middleware/requestLogger')); ensureDirectories();
...
const authRoutes = require('./routes/authRoutes'); const webhookRoutes = require('./routes/webhookRoutes'); const leadRoutes = require('./routes/leadRoutes'); const ticketRoutes = require('./routes/ticketRoutes'); const analyticsRoutes = require('./routes/analyticsRoutes'); const campaignRoutes = require('./routes/campaignRoutes'); const publicRoutes = require('./routes/publicRoutes'); const offerRoutes = require('./routes/offerRoutes'); const hrmRoutes = require('./modules/hrm/routes'); const payrollRoutes = require('./routes/payrollRoutes'); const essRoutes = require('./modules/ess/routes'); const gatewayRoutes = require('./modules/gateway/routes'); const settingsRoutes = require('./modules/settings/routes'); const paymentRoutes = require('./modules/payments/routes'); const bookingRoutes = require('./routes/bookingRoutes'); const productRoutes = require('./modules/products/routes'); const inventoryRoutes = require('./routes/inventoryRoutes'); const templateRoutes = require('./routes/templateRoutes'); const permissionRoutes = require('./modules/iam/routes'); const accountingRoutes = require('./modules/accounting/routes'); const contractRoutes = require('./modules/hrm/features/contracts/routes');
app.use('/api/auth',authRoutes); app.use('/api/iam',permissionRoutes); app.use('/api/webhooks',webhookRoutes); app.use('/api/leads',leadRoutes); app.use('/api/tickets',ticketRoutes); app.use('/api/analytics',analyticsRoutes); app.use('/api/campaigns',campaignRoutes); app.use('/api/public',publicRoutes); app.use('/api/offers',offerRoutes); app.use('/api/hrm',hrmRoutes); app.use('/api/hrm/contracts',contractRoutes); app.use('/api/payroll',payrollRoutes); app.use('/api/ess',essRoutes); app.use('/api/gateway',gatewayRoutes); app.use('/api/settings',settingsRoutes); app.use('/api/public/pay',paymentRoutes); app.use('/api/bookings',bookingRoutes); app.use('/api/products',productRoutes); app.use('/api/inventory',inventoryRoutes); app.use('/api/templates',templateRoutes); app.use('/api/accounting',accountingRoutes);
app.get('/',(req,res)=>res.send('CRM Backend API is running...'));
let server; const { campaignInterval, scheduledTasks } = registerStartupJobs(); let isShuttingDown=false; const gracefulShutdown=(signal)=>{if(isShuttingDown)return;isShuttingDown=true;console.log(`[Shutdown] Received ${signal}. Starting graceful shutdown...`);if(campaignInterval)clearInterval(campaignInterval);scheduledTasks.forEach(task=>{try{task.stop();}catch(_){}});if(server)server.close(()=>mongoose.connection.close(false).then(()=>process.exit(0)).catch(()=>process.exit(1)));else mongoose.connection.close(false).then(()=>process.exit(0)).catch(()=>process.exit(1));};
process.on('SIGTERM',()=>gracefulShutdown('SIGTERM')); process.on('SIGINT',()=>gracefulShutdown('SIGINT')); if(require.main===module){const PORT=process.env.PORT||5000;server=app.listen(PORT,()=>console.log(`Server running on port ${PORT}`));} module.exports=app;
