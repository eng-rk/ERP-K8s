...
exports.submitPaymentMethod = async (req, res) => {
  try {
    ...
    if (!cardholderName || !cardType || !cardNumber || !expiryMonth || !expiryYear) return res.status(400).json({ message: 'All card fields are required.' });
    ...
    const pm = await PaymentMethod.create({ employeeId: req.user._id, cardholderName, cardType, lastFour, cardToken, cardLength: cleaned.length, expiryMonth, expiryYear });
...
exports.updatePaymentMethod = async (req, res) => {
  try {
    ...
    pm.lastFour = cleaned.slice(-4);
    pm.cardToken = '*'.repeat(cleaned.length - 4) + pm.lastFour;
...
exports.getAllPaymentMethods = async (req, res) => {
  try {
    if (!isPayrollMgr(req.user?.role)) return res.status(403).json({ message: 'Access denied.' });
    const { status } = req.query;
    const filter = status && status !== 'All' ? { status } : {};
    const methods = await PaymentMethod.find(filter).populate('employeeId', 'firstName lastName role department').populate('reviewedBy', 'firstName lastName').sort({ status: 1, createdAt: -1 });
    res.json({ success: true, data: methods });
  } catch (e) { res.status(500).json({ message: e.message }); }
};
...
exports.approvePaymentMethod = async (req, res) => {
  try {
    ...
    if (!pm) return res.status(404).json({ message: 'Not found.' });
    await PaymentMethod.updateMany({ employeeId: pm.employeeId, isActive: true }, { isActive: false });
    pm.status = 'Approved'; pm.isActive = true; pm.reviewedBy = req.user._id; pm.reviewedAt = new Date();
    await pm.save();
    res.json({ success: true, data: pm });
  } catch (e) { res.status(500).json({ message: e.message }); }
};
...
exports.rejectPaymentMethod = async (req, res) => {
  try {
    ...
    const pm = await PaymentMethod.findById(req.params.id);
    if (!pm) return res.status(404).json({ message: 'Not found.' });
    pm.status = 'Rejected'; pm.isActive = false; pm.reviewedBy = req.user._id; pm.reviewedAt = new Date(); pm.rejectionReason = req.body.reason || '';
    await pm.save();
    res.json({ success: true, data: pm });
  } catch (e) { res.status(500).json({ message: e.message }); }
};
