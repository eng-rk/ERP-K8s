...
exports.getBookingByRef = async (req, res) => {
  try {
    ...
    const booking = await Booking.findOne({ $or: [{ bookingRef: ref }, { recordLocator: ref }] })
...
exports.getBookings = async (req, res) => {
  try {
    ...
    const bookings = await Booking.find(filter)
      .populate('offer', 'title price status').populate('lead', 'name email').populate('agent', 'firstName lastName').sort({ createdAt: -1 });
...
exports.updateBooking = async (req, res) => {
  try {
    ...
    if (!isAdmin && booking.agent.toString() !== req.user._id.toString()) return res.status(403).json({ message: 'Not authorized to update this booking' });
    ...
    if (req.body.status === 'Canceled' || req.body.status === 'Refunded') {
      ...
      } catch (cascadeErr) { console.error('Failed to cascade booking status:', cascadeErr.message); }
    }
    res.json({ success: true, data: booking });
  } catch (error) { res.status(500).json({ message: 'Server Error', error: error.message }); }
};
