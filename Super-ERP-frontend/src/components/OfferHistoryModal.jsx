      ...
      <div style={{ background: 'var(--bg-card)', borderRadius: 12, padding: 32, maxWidth: 640, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.3)', maxHeight: '80vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            {onViewVersions && <button onClick={onViewVersions} style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border-color)', borderRadius: 8, padding: '4px 10px', fontSize: 12, cursor: 'pointer', color: 'var(--accent-primary)' }}><Icon name="send" /> Sent Versions</button>}
        ...
        {loading ? <div className="loading-state"><div className="spinner" />Loading history...</div> : history.length === 0 ? <div className="empty-state"><p>No history yet.</p></div> : <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>{history.map((entry, idx) => <div key={entry._id || idx} style={{ display: 'flex', gap: 16, padding: '12px 0', borderBottom: idx < history.length - 1 ? '1px solid var(--border-color)' : 'none' }}><div style={{ fontSize: 20, width: 32, textAlign: 'center' }}>{getActionIcon(entry.action)}</div><div style={{ flex: 1 }}><div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}><span style={{ fontWeight: 600, fontSize: 14, textTransform: 'capitalize' }}>{entry.action.replace('_', ' ')}</span>{entry.version != null && <span className="badge badge-meta" style={{ fontSize: 10, padding: '1px 7px', fontWeight: 700 }}>v{entry.version}</span>}</div><div style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 2 }}>{entry.details}</div>{renderChanges(entry.changes)}<div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>{entry.performedBy ? `${entry.performedBy.firstName} ${entry.performedBy.lastName}` : 'System'} • {new Date(entry.timestamp).toLocaleString()}</div></div></div>)}</div>}
      </div>
    </div>
  );
};
