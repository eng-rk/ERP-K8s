...
const OfferDetailPage = () => {
  ...
  const fetchOffer = async () => {
    try {
      ...
      const leadId = offerData?.lead?._id || offerData?.lead;
      if (leadId) {
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: 10 }}>
                {offer.images.map(img => img && img.url && (
                  <div key={img._id || img.url} style={{ position: 'relative', borderRadius: 10, overflow: 'hidden', border: '1px solid var(--border-color)', background: '#000', minHeight: 110 }}>
                    <img src={img.url.startsWith('http') || img.url.startsWith('data:') ? img.url : `${API_HOST}${img.url}`} alt={img.caption || 'Offer image'} style={{ width: '100%', height: 110, objectFit: 'cover' }} />
