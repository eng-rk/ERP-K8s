...
import { User, Shield, ArrowLeft, Edit3, Send, Zap, CheckCircle, AlertTriangle } from 'lucide-react';
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 20, borderBottom: '1px solid var(--border-color)', paddingBottom: 10 }}>
        ...
        >
          <Shield size={14} />
          <span>Enterprise Permissions Matrix</span>
        </button>
      </div>
      ...
      {activeTab === 'permissions' ? (
        <div className="table-wrapper" style={{ padding: 20 }}>
          <PermissionsTab userId={id} readOnly={!isAdmin} />
        </div>
      ) : (
            ...
            {editing ? (
              ...
              <>
                ...
                <div className="table-wrapper" style={{ padding: 24 }}>
                  ...
                  {user?.smtpUser && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', background: 'rgba(37,99,235,0.08)', borderRadius: 8, marginBottom: 14, fontSize: 13 }}>
                      <Send size={16} />
                  ...
                  <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 16 }}>
                    {isOwnProfile
                      ...
                      : `Configure ${user?.firstName} ${user?.lastName}'s SMTP so they can send offer emails directly from their account.`}
                  </p>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                    <div className="form-group" style={{ margin: 0, gridColumn: '1 / -1' }}>
                  ...
                  <div style={{ marginTop: 16, display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                    ...
                    {smtpTestResult && (
                      <span style={{ fontSize: 13, fontWeight: 600, color: smtpTestResult.success ? 'var(--accent-success)' : 'var(--accent-danger)', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                        {smtpTestResult.success ? <CheckCircle size={14} /> : <AlertTriangle size={14} />}
                ...
                {isAdmin && (
                  <>
                    <div className="table-wrapper" style={{ padding: 24 }}>
                      <div className="table-title" style={{ marginBottom: 20 }}>Role & Status</div>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                        <div className="form-group" style={{ margin: 0 }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                        {Object.entries(permissionLabels).map(([key, label]) => (
                          <label key={key} style={{
                            ...
                            background: form.permissions[key] ? 'rgba(34,197,94,0.08)' : 'var(--bg-card-hover)',
                    ...
                    {Object.keys(permissionLabels).map((key) => {
                      ...
                      return (
                        <div key={key} style={{
                          ...
                          background: granted ? 'rgba(34,197,94,0.08)' : 'var(--bg-card-hover)',
                          border: `1px solid ${granted ? 'rgba(34,197,94,0.2)' : 'var(--border-color)'}`,
                        }}>
                          <div style={{
                            width: 8, height: 8, borderRadius: '50%',
                            background: granted ? 'var(--accent-success)' : 'var(--text-muted)',
                            flexShrink: 0,
                          }} />
