...
const ExecutiveDashboardPage = () => {
  ...
  useEffect(() => {
    ...
    ]).then(([analyticsRes, marketingRes]) => {
          ...
          {marketingData && (
            <div className="card" style={{ marginBottom: 20 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 20 }}>
                <ResponsiveContainer width="100%" height={260}>
                  <BarChart data={marketingData.performanceData} margin={{ top: 8, right: 16, left: -20, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="var(--border-color)" vertical={false} />
                    <XAxis dataKey="platform" tick={{ fill: 'var(--text-muted)', fontSize: 12 }} axisLine={false} tickLine={false} />
                    <YAxis tick={{ fill: 'var(--text-muted)', fontSize: 12 }} axisLine={false} tickLine={false} />
                    <Tooltip content={<ChartTooltip />} cursor={{ fill: 'var(--bg-body)' }} />
                    <Legend wrapperStyle={{ fontSize: 12, color: 'var(--text-secondary)' }} />
                    <Bar dataKey="totalLeads" name="Total Leads" fill={C.blue} radius={[4, 4, 0, 0]} />
                    <Bar dataKey="convertedLeads" name="Confirmed Leads" fill={C.green} radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>

                <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                  {marketingData.winningPlatform ? (
                    <div style={{
                     ...
                     <div style={{ padding: 16, textAlign: 'center', color: 'var(--text-muted)' }}>Not enough data for smart recommendations.</div>
                  )}
                </div>
              </div>
            </div>
